...
export default function Recap2025Section() {
  ...
  return (
    <section id="2025-recap" className="py-32 relative bg-slate-900">
        ...
        <div className="relative mb-20 group">
          <div className="relative aspect-video rounded-lg overflow-hidden bg-slate-800">
            ...
            <button
              ...
              className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-black/50 rounded-full flex items-center justify-center text-white opacity-0 group-hover:opacity-100 transition-opacity"
            >
              <ChevronLeft size={20} />
            </button>
            <button
              onClick={() => setCurrentSlide((prev) => (prev + 1) % photoSlides.length)}
              className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-black/50 rounded-full flex items-center justify-center text-white opacity-0 group-hover:opacity-100 transition-opacity"
            >
            ...
            {winners.map((winner, index) => (
              <div key={index} className="flex items-baseline justify-between border-b border-white/10 pb-6">
                <div className="flex items-baseline gap-6">
                  <span className="text-3xl font-bold text-white w-12">{winner.place}</span>
